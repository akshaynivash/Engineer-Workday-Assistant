...
def find_alternative_parts_balanced(selected_part, df, min_alternatives=5, relaxation_tier="Tier 1"):
    ...
    selected_fuse_type = (
        selected_part["Attribut1"].strip().lower() if pd.notna(selected_part["Attribut1"]) else None
    )
    ...
    df["Rated Current (A)"] = pd.to_numeric(
        df["Rated Current (A)"].astype(str).str.replace("A", ""), errors="coerce"
    )
    df["Rated Voltage (V)"] = pd.to_numeric(
        df["Rated Voltage (V)"].astype(str).str.replace("V", ""), errors="coerce"
    )
    if "Rated Voltage (VDC)" in df.columns:
        df["Rated Voltage (VDC)"] = pd.to_numeric(
            df["Rated Voltage (VDC)"].astype(str).str.replace("V", ""), errors="coerce"
        )

    df["Rated Breaking Capacity (A)"] = pd.to_numeric(
        df["Rated Breaking Capacity (A)"].astype(str).str.replace("A", ""), errors="coerce"
    )
    ...
    df["Rated Breaking Capacity (A)"] = df["Rated Breaking Capacity (A)"].fillna(
        df["Rated Breaking Capacity (A)"].median()
    )

    # Convert selected values to float safely
    selected_voltage_ac = pd.to_numeric(str(selected_voltage_ac).replace("V", ""), errors="coerce")
    selected_voltage_dc = (
        pd.to_numeric(str(selected_voltage_dc).replace("V", ""), errors="coerce") if selected_voltage_dc else None
    )
    ...
    if relaxation_tier == "Tier 1":
        alternatives = df[
            ...
            & (
                df["Rated Breaking Capacity (A)"].between(
                    selected_breaking_capacity * 0.9, selected_breaking_capacity * 1.1
                )
            )
    ...
    # 🛠 **Tier 2: Allow Minor Breaking Capacity Variations (mounting no longer required)**
        ...
        alternatives = df[
            ...
            & (
                df["Rated Breaking Capacity (A)"].between(
                    selected_breaking_capacity * 0.8, selected_breaking_capacity * 1.2
                )
            )
        ]
